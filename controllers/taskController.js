...
router.get("/get-todo-project", (req, res) => {
  //db.query('SELECT * FROM projects', (err, rows) => {
  db.query(
    'SELECT projects.Name, projects.id, GROUP_CONCAT( json_object( "tasksID", tasks.tasksID, "Title", tasks.Title, "status" ,tasks.status, "project_id", tasks.project_id ) )task FROM projects LEFT JOIN tasks ON tasks.project_id = projects.id GROUP BY projects.id',
    (err, rows) => {
      if (err) throw err;
