...
function getTodoProject() {
    fetch('/get-todo-project',
        ...
        for (let i = 0; i < showBody.length; i++) {
            ...
            out += `<div id="editor"><textarea data-edit_area_id="${showBody[i]['id']}" name="textHeader" class="textarea-edit" cols="50" autofocus></textarea>`;
            out += `</div>`;
            out += `<h3>${showBody[i]['Name']}</h3>`;
            out += `<ul class="hidden"><li><a class="todos-action-edit" href="#" data-edit_todo_id="${showBody[i]['id']}" id="edit-${showBody[i]['id']}" title="Edit">Edit</a></li>`;
            out += `<li><a class="todos-action-remove" data-delete_todo_id="${showBody[i]['id']}" id="${showBody[i]['id']}" href="#" title="Delete">Delete</a></li></ul>`
            out += `</div>`;
            ...
            for (let j = 0; j < showTask.length; j++) {
                ...
                if (showTask[j]['project_id'] != null) {
                    if (showTask[j]['status'] == 'false') {

                        out += `<tr data-task-id="${showTask[j]['tasksID']}" class="completed-task">`;
                    ...
                    out += `<td class="todo-list-task"><p>${showTask[j]['Title']}</p></td>`;
...
document.addEventListener('DOMContentLoaded', function () {
    window.onload = (function () {
        ...
        $(".todos-action-edit").click(function () {
            ...
            let editText = $(this).parent().parent().prev();
        ...
        $("form > button").click(function () {
            let formActionId = $(this).parent().data("action-form");
            let areaTextTask = $(this).parent().find('input');
...
$(this).parent().find('input').val().html = ''
        ...
        $(".todo-list-checkbox input").click(function () {
            let completeId = $(this).parent().parent().data('task-id');
            ...
            if ($(this).is(":checked")) {
                ...
                let checkboxTrue = $(this).parent().parent().addClass('completed-task');

            } else if ($(this).is(":not(:checked)")) {
                let checkboxFalse = $(this).parent().parent().removeClass('completed-task');
                $.ajax({
                    url: "/todo/complete-task",
                    type: "POST",
